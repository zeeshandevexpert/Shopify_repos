...
(function(){
  ...
  theme.CartForm = (function() {
    var selectors = {
      ...
      customCartCount: '.custom-cart--count',
      cartNote: '[name="note"]',
    ...
    CartForm.prototype = Object.assign({}, CartForm.prototype, {
      ...
      updateBuildCart:function(cart_final){
         theme.cart.getCartProductMarkup().then(cart_final.cartMarkup.bind(cart_final));
       // cart_final.customBar();
      },
      
      
      buildCart: function() {
      var cart_final = this;
      var update_anything = false;
      $.get("/cart.json",function(res){
        var item_count = res['item_count'];
      var items = res['items'];
            if(item_count >0){
            var free_wallet_exist = false;
            var free_jeweller_exist = false;
            var freewallet = document.querySelector('.freeproduct2').getAttribute('data-variant-id');
              var freejewllery = document.querySelector('.freeproduct3').getAttribute('data-variant-id');
              console.log('freewallet = '+freewallet);
              console.log('freejewllery = '+freejewllery);
              var walletline = '';
              var jewlleryline = '';
              ...
              for(var i=0;i<items.length;i++){
                var variant_id = items[i]['variant_id'];
                          if(variant_id == freewallet || variant_id == freejewllery){
                                if(variant_id == freewallet){
                                  free_wallet_exist = true;
                                  walletline = i;
                                }
                                if(variant_id == freejewllery){
                                free_jeweller_exist = true;  
                                  jewlleryline = i;
                                }
                          }
                else{
                  actual_count = parseInt(actual_count)+parseInt(items[i]['quantity']);
                }
              }
              ...
              console.log('actual_count = '+actual_count);
                 ...
                 if(actual_count ==1){
                    cart_final.finalcustomBar("57",actual_count);
                          if(free_wallet_exist == true || free_jeweller_exist == true){
                            ...
                            var updates = {
                                ...
                                .then(response => {
                                            cart_final.updateBuildCart(cart_final);
                                  cart_final.finalcustomBar("57",actual_count);
                        ...
                        if(actual_count >=2 && actual_count <=4){
                          cart_final.finalcustomBar("85",actual_count);
                                  if(free_wallet_exist == true){
                                    if(free_jeweller_exist == true){
                                      update_anything = true;
                        ...
                        else if(actual_count >=5){
                          cart_final.finalcustomBar("100",actual_count);
                         if(free_wallet_exist == true && free_jeweller_exist == true){
                                  update_anything = true;
                                    var updates = {
                                  [freewallet]: 0
                        ...
                        else if(free_wallet_exist == true && free_jeweller_exist == false){
                          ...
                          else if(free_wallet_exist == false && free_jeweller_exist == false){
                                 update_anything = true;
                                var formData = {
                                     'items': [{
                                      'id': freejewllery,
          ...
          var actual_count = 0;
                cart_final.finalcustomBar("0",actual_count);
        }
        
          if(update_anything == false){
            cart_final.updateBuildCart(cart_final);
          }
      ...
      finalcustomBar:function(width,actual_count){
            fetch(window.location.pathname + "?sections=custom-bar")
          .then(res => res.json())
          .then(res => {
            console.log(res);
            const bar = new DOMParser().parseFromString(res["custom-bar"], 'text/html');
            document.querySelector("#custom-ship-bar").innerHTML = bar.querySelector("#shopify-section-custom-bar").innerHTML;
             $('.custom-drawer-bar-fill').css('width',width+'%');
    
            if(actual_count == 0){
              $(".custom-drawer-bar-away-msg").hide();
            } 
            else{
			...
			if(actual_count <= 1) {
			$('.empty-cart-text').addClass('show-emptytext');
			}
			else if(actual_count == 2) {
			$('.custom-cart-text1').addClass('show-text1');
			$('.custom-cart-text1 span').text('3');
              $(".custom-drawer-bar-away-msg").show();
			}
			else if(actual_count == 3) {
			$('.custom-cart-text1').addClass('show-text1');
			$('.custom-cart-text1 span').text('2');
              $(".custom-drawer-bar-away-msg").show();
			}
			else if(actual_count == 4) {
			$('.custom-cart-text1').addClass('show-text1');
			$('.custom-cart-text1 span').text('1');
              $(".custom-drawer-bar-away-msg").show();
			}
			else if(actual_count > 4) {
			$('.custom-cart-text2').addClass('show-text2');
              $(".custom-drawer-bar-away-msg").show();
			}
            console.log(bar);
        });
      ...
      customBar: function(){
        ...
        console.log("testttttt entry cart thing");
      //  this.updateCartFreeGifts();
      
      },
      
//         updateCartFreeGifts:function(){
//           console.log("update free gifts");
//           var total_defaultqty = 0;
//           var freeproduct2 = $('.freeproduct2').data('variant-id'); 
//       var freeproduct3 = $('.freeproduct3').data('variant-id');
// var addlineItems = [
//     { id: freeproduct3, quantity: 1 },
//     { id: freeproduct2, quantity: 1 }
// ];
                  
//           setTimeout(function() {
//             var total_free2qty = $('.cart__items .cart__item.hide-qty[data-variant-id="'+freeproduct2+'"]').length;
//           var total_free3qty = $('.cart__items .cart__item.hide-qty[data-variant-id="'+freeproduct3+'"]').length;
//             var datakey_2 = $('.cart__items .cart__item.hide-qty[data-variant-id="'+freeproduct2+'"]').data('key');
//           var datakey_3 = $('.cart__items .cart__item.hide-qty[data-variant-id="'+freeproduct3+'"]').data('key');
//           $('.cart__items .cart__item.show-qty').each(function() {
//             var defaultqty = $(this).data('qty');
//             total_defaultqty = defaultqty + total_defaultqty;
//           });
//             console.log('total_defaultqty = '+total_defaultqty);
//             if (total_defaultqty == 0) {
//               var removelineItems = [
//     { id: freeproduct3, quantity: 0 },
//     { id: freeproduct2, quantity: 0 }
// ];
//             $('.custom-drawer-bar-fill').css('width','0%');
//             $('.empty-cart-text').show();
//               if(total_free3qty == 0 && total_free2qty == 0) {
//               var send_data = $.post("/cart/change",{ items: removelineItems });
//             send_data.done(function(res){
//               document.dispatchEvent(new CustomEvent('cart:build'));
//             });
//               }
//             else if(total_free3qty == 0 && total_free2qty > 0) {
//                   var send_data = $.post("/cart/change","id="+freeproduct2+"&quantity=0");
//             send_data.done(function(res){
//               document.dispatchEvent(new CustomEvent('cart:build'));
//             });
//                 }
//                   else if(total_free3qty > 0 && total_free2qty == 0) {
//                   var send_data = $.post("/cart/change","id="+freeproduct3+"&quantity=0");
//             send_data.done(function(res){
//               document.dispatchEvent(new CustomEvent('cart:build'));
//             });
//                 }
//               else if(total_free3qty > 0 && total_free2qty > 0){
//                 console.log('both exist 0');
//                 var send_data = $.post("/cart/change",{ items: removelineItems });
//             send_data.done(function(res){
//               document.dispatchEvent(new CustomEvent('cart:build'));
//             });
//               }
//             }
//           else if (total_defaultqty == 1) {
//             var removelineItems = [
//     { id: freeproduct3, quantity: 0 },
//     { id: freeproduct2, quantity: 0 }
// ];
            
//             $('.custom-drawer-bar-fill').css('width','57%');
//             $('.empty-cart-text').show();
//             if(total_free3qty == 0 && total_free2qty == 0) {
//               var send_data = $.post("/cart/change",{ items: removelineItems });
//             send_data.done(function(res){
...
//             }
//           }
//             else if(total_defaultqty == 3) {
//               $('.custom-drawer-bar-fill').css('width','85%');
//               $('.custom-cart-text1').show();
//             $('.custom-cart-text1 span').text('2');
//               var send_data = $.post("/cart/change","id="+freeproduct3+"&quantity=0");
//             send_data.done(function(res){
//               document.dispatchEvent(new CustomEvent('cart:build'));
...
//             });
//             }
//           else if(total_defaultqty > 1 && total_defaultqty < 5) {
//             $('.custom-drawer-bar-fill').css('width','85%');
//             $('.custom-cart-text1').show();
//             if(total_free2qty == 0) {
//             var send_data = $.post("/cart/add","id="+freeproduct2+"&quantity=1");
//             send_data.done(function(res){
//               document.dispatchEvent(new CustomEvent('cart:build'));
//             });
//             }
...
//           }
//             else if(total_defaultqty > 4) {
//               $('.custom-drawer-bar-fill').css('width','100%');
//               $('.custom-cart-text2').show();
//               if(total_free3qty == 0 && total_free2qty == 0) {
//               var send_data = $.post("/cart/add",{ items: addlineItems });
//             send_data.done(function(res){
//               document.dispatchEvent(new CustomEvent('cart:build'));
//             });
//               }
//                 else if(total_free3qty == 0 && total_free2qty > 0) {
//                   var send_data = $.post("/cart/add","id="+freeproduct3+"&quantity=1");
//             send_data.done(function(res){
//               document.dispatchEvent(new CustomEvent('cart:build'));
//             });
//                 }
...
//         },
      ...
      updateCount: function(count) {
        ...
        var cCount = document.querySelectorAll(selectors.customCartCount);
        if(cCount.length) {
            cCount.forEach(b => {
              b.innerText = count;
            });
  ...
  theme.buildProductGridItem = function(items, gridWidth, rowOf, imageSizes) {
    ...
    items.forEach(product => {
      ...
      let customFirstImage = ""
      let customSecondImage = ""
      let customPrice = ""
      let customComparePrice = ""
      if(product.tags.includes("NEW")){
        customNewBadge = "<li>NEW</li>";
      }
      if(product.compare_at_price > product.price){
        let customOffPrice = product.compare_at_price - product.price;
        customPriceBadge = `<li>${Shopify.formatMoney(customOffPrice)} OFF</li>`;
        customComparePrice = `<strike>${Shopify.formatMoney(product.compare_at_price)}</strike>`;
      }
      if(product.images.length > 0){
        customFirstImage = `<img src="${product.images[0]}" class="custom-main-collection-grid-image"/>`
        if(product.images.length > 1){
          customSecondImage = `<img src="${product.images[1]}" class="custom-main-collection-grid-image-second" />`;
        }
      }
      var markup = `<swiper-slide>
        <a href="${product.url}" class="custom-main-collection-grid">
          <div class="custom-main-collection-grid-images-wrapper">
            <ul class="custom-main-collection-grid-badges">
              ${customNewBadge}
              ${customPriceBadge}
            </ul>
            <div class="custom-main-collection-grid-image-wrapper">
              ${customFirstImage}
              ${customSecondImage}
            </div>
          </div>
          <div class="custom-main-collection-grid-info">
            <p class="custom-main-collection-grid-title">${product.title}</p>
            <p class="custom-main-collection-price">
              <span><span class="currency-symbol sale-price"></span>
              ${theme.Currency.formatMoney(product.price)}
              </span>
              <span class="currency-symbol compare-price"></span>${customComparePrice}
            </p>
          </div>
        </a>
        </swiper-slide>
